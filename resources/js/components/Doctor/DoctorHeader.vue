...
defineProps<{
    ...
    resultsCount: number;
    showForm: boolean;
    tienePermisoCrearOrEditar: boolean;
    esDoctorOrAdmin: boolean;
}>();
...
<template>
    <div class="flex flex-col gap-4 rounded-3xl border p-6 shadow-[0_15px_40px_rgba(0,169,157,0.04)] border-[var(--border)] bg-[var(--card)] lg:flex-row lg:items-center lg:justify-between transition-colors duration-300">
        <div>
            ...
            <p class="text-[length:var(--font-xs)] leading-5 text-[var(--muted)] mt-1">
                Busca, visualiza, registra y administra doctores del sistema.
            </p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                <span v-if="!esDoctorOrAdmin"
                    class="inline-flex items-center rounded-2xl bg-[var(--section-soft)] px-4 py-3 text-xs font-bold text-[var(--muted)] border border-[var(--border)]">
                    {{ resultsCount }} resultado(s)
                </span>

                <button v-if="tienePermisoCrearOrEditar" type="button" @click="emit('toggle-form')"
